...
resource "google_logging_project_sink" "log_sink_cdc" {
  ...
  filter = format("protoPayload.methodName=google.cloud.bigquery.v2.JobService.InsertJob AND protoPayload.authorizationInfo[0].permission=bigquery.tables.updateData AND protoPayload.resourceName = projects/%s/datasets/%s/tables/%s", var.gcp_project_id, var.bigquery_dataset_name, var.bigquery_table_name)
